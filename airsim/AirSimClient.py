...
class Vector3r(MsgpackMixin):
    ...
    def init_from_string(self, str):
        self.x_val, self.y_val, self.z_val = [np.float32(item) for item in str.split(',')]
        return self

    def __str__(self):
        return 'Vector3r({})'.format(self.to_string())

    def to_string(self):  # for mininet-wifi position format
        PRECISION = 2
        return '{},{},{}'.format(
                round(self.x_val, PRECISION),
                round(self.y_val, PRECISION),
                round(self.z_val, PRECISION))

    def length(self):
        return np.sqrt(self.x_val**2 + self.y_val**2 + self.z_val**2)

    def add(self, x_val, y_val, z_val):
        self.x_val += x_val
        self.y_val += y_val
        self.z_val += z_val
        return self
